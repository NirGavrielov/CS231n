...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        'W2' : np.random.normal(0, weight_scale, size=(int(num_filters * np.prod(input_dim[1:])/4), hidden_dim)), # 1/4 due to pooling
    ...
    def loss(self, X, y=None):
        ...
        aff_relu_out, aff_relu_cache = affine_relu_forward(conv_out, W2, b2)
        ...
        for i in range(1,4): 
            loss += .5 * self.reg * np.sum(self.params[f'W{i}'] ** 2)
