...
def rnn_step_backward(dnext_h, cache):
    ...
    # get before activation gradient from tanh derivative (sech^2x = 1-tanh^2x)
    (x, prev_h, Wx, Wh, b, next_h) = cache
    dz = dnext_h * (1 - next_h ** 2)

    # extract all gradients like in affine layers
    dx = (dz @ Wx.T).reshape(x.shape)
    dWx = x.T @ dz
    dprev_h = (dz @ Wh.T).reshape(prev_h.shape)
    dWh = prev_h.T @ dz
    db = dz.sum(axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for step in range(T):
        h_T, cache_T = rnn_step_forward(x[:, step], h[-1], Wx, Wh, b)
        h.append(h_T)
        cache.append(cache_T)
    h.pop(0)  # remove h0 from the list
    h = np.array(h).swapaxes(0, 1)  # to fix the correct dimensionality
...
def rnn_backward(dh, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, T, H = dh.shape
    D = cache[0][0].shape[1]  # get D from first x cache
    dx = np.zeros((N, T, D))
    dh0 = np.zeros((N, H))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    ...
    for step in reversed(range(T)):
        dx[:, step], dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, step] + dh0, cache[step])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, Wx, Wh, a, i, f, o, g, tanh_c)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    (x, prev_h, prev_c, Wx, Wh, a, i, f, o, g, tanh_c) = cache
    do = dnext_h * tanh_c
    dnext_c += dnext_h * o * (1 - tanh_c ** 2)  # backprop mul. with o & tanh
    di = dnext_c * g
    dg = dnext_c * i
    df = dnext_c * prev_c
    dprev_c = dnext_c * f
    ...
    da = np.concatenate((di * i * (1 - i),
                         df * f * (1 - f),
                         do * o * (1 - o),
                         dg * (1 - g ** 2)), axis=1)  # 3 sigmoids and tanh

    dx = (da @ Wx.T).reshape(x.shape)
    dWx = x.T @ da
    dprev_h = (da @ Wh.T).reshape(prev_h.shape)
    dWh = prev_h.T @ da
    db = da.sum(axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h, cache, c_T = [h0], list(), np.zeros_like(h0)
    T = x.shape[1]  # number of time steps
    for step in range(T):
        h_T, c_T, cache_T = lstm_step_forward(x[:, step], h[-1], c_T,  Wx, Wh, b)
        h.append(h_T)
        cache.append(cache_T)
    h.pop(0)  # remove h0 from the list
    h = np.array(h).swapaxes(0, 1)  # to fix the correct dimensionality
...
def lstm_backward(dh, cache):
    ...
    for step in reversed(range(T)):
        dx[:, step], dh0, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, step] + dh0, dc, cache[step])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
