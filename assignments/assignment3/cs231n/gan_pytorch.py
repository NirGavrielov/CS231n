...
def discriminator(seed=None):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    d1, d2, d3 = 784, 256, 1  # according to Generative_Adversarial_Networks.ipynb
...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    d1, d2 = 1024, 784  # according to Generative_Adversarial_Networks.ipynb
    model = nn.Sequential(nn.Linear(noise_dim, d1),
                          nn.ReLU(),
                          nn.Linear(d1, d1),
                          nn.ReLU(),
                          nn.Linear(d1, d2),
...
def bce_loss(input, target):
    ...
    # TODO nir: replaced course code with stable version due to dimensions error
...
def generator_loss(logits_fake):
    ...
    fake_labels = torch.ones_like(logits_fake)  # generator aims to maximize wrong scores
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = .5 * torch.mean(((scores_real - 1) ** 2 + scores_fake ** 2), dim=0)
...
def ls_generator_loss(scores_fake):
    ...
    loss = .5 * torch.mean((scores_fake - 1) ** 2, dim=0)
