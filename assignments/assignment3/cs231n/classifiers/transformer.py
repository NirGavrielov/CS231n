...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embedded_positioned = self.positional_encoding(self.embedding(captions))  # dim = [N, T, W]
        projected_features = self.visual_projection(features).unsqueeze(1)  # dim = [N, 1, W]
        tgt_mask = torch.tril(torch.ones((T, T)))
        output = self.transformer(embedded_positioned, projected_features, tgt_mask)
