...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        normalization_k = torch.tensor([sim(z_k, out[j]) / tau for j in range(len(out)) if j != k]).exp().sum()
        normalization_k_n = torch.tensor([sim(z_k_N, out[j]) / tau for j in range(len(out)) if j != k + N]).exp().sum()

        l1 = -((sim(z_k, z_k_N) / tau).exp() / normalization_k).log()
        l2 = -((sim(z_k_N, z_k) / tau).exp() / normalization_k_n).log()
        total_loss += l1 + l2
...
def sim_positive_pairs(out_left, out_right):
    ...
    normalized_left = out_left / torch.linalg.norm(out_left, axis=1, keepdim=True)
    normalized_right = out_right / torch.linalg.norm(out_right, axis=1, keepdim=True)
    pos_pairs = torch.sum(normalized_left * normalized_right, dim=1, keepdim=True)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    simalirity = sim_matrix[range(N), range(N, 2 * N)].repeat(2) # sim is symmetric k,N+k <-> N+k, k
